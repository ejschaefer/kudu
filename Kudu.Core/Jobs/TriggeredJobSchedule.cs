...
namespace Kudu.Core.Jobs
{
    public class TriggeredJobSchedule : IDisposable
    {
        ...
        public void Reschedule(DateTime lastRun, Schedule schedule = null)
        {
            ...
            var nextOccurence = Schedule.GetNextSchedule(lastRun);

            Logger.LogInformation("Scheduling WebJob with " + schedule + " next schedule expected in " + nextOccurence);

            _timer.Change(nextOccurence, Timeout.InfiniteTimeSpan);
        }
